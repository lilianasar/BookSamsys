...
export class Book extends Component {
  ...
  ChangeUrl(){
    ...
    //let currentPage = this.setState(2)
    //let quantity = this.setState(5)
  }
    ...
    var filteredData = this.state.booksWithoutFilter.filter(function (el) {
      return (
        el.id
          .toString()
          .toLowerCase()
          .includes(IdFilter.toString().trim().toLowerCase()) &&
        el.isbn
          .toString()
          .toLowerCase()
          .includes(IsbnFilter.toString().trim().toLowerCase()) &&
        el.nome
          .toString()
          .toLowerCase()
          .includes(NomeFilter.toString().trim().toLowerCase()) &&
        el.autor
          .toString()
          .toLowerCase()
          .includes(AutorFilter.toString().trim().toLowerCase()) &&
        el.preco
          .toString()
          .toLowerCase()
          .includes(PrecoFilter.toString().trim().toLowerCase())
      );
    });
  ...
  handleClick = (novaPagina) => {
    this.setState({ currentPage: novaPagina });
  };

 
  //function Table(){
  refreshList() {
    fetch(variables.API_URL + `book?pageNumber=${1}&pageQuantity=${5}`) ///pg?pageNumber=0&pageQuantity=5")
      .then((response) => response.json())
      .then((data) => {
        this.setState({ books: data.obj.books, booksWithoutFilter: data.obj.books });
  ...
  changeAnt = () => {
    //var newC= this.currentPage -1;
    //this.setState({currentPage : this.currentPage + 1})
   //this.setState(st => {
   //   st.currentPage = this.currentPage-1
    //})
  ...
  deleteClick(id) {
    if (window.confirm("Deseja mesmo eliminar o livro?")) {
      fetch(variables.API_URL + "book/?id=" + id, {
        //?id=1'
        method: "DELETE",
        headers: {
          Accept: "application/json",
  ...
  render() {
    const { books, pages, modalTitle, id, isbn, nome, autor, preco } = this.state;
    var {currentPage, quantity} = this.state;
    ...
    return (
      <div>
        ...
        <select onChange={this.changeQuants}  defaultValue={"5"}>
        ...
        <div>Total {this.totalCount}</div>
        <div>
        {this.currentPage > 1 && (
        <button type="button" className="btn btn-light"
        //() => this.sortResult("preco", false)
        onClick={() => this.changeAnt(currentPage)}
        >Anterior</button>
        )}
        {pages.map(page => (
            <button type="button" className="btn btn-light"
            //isSelect={page === this.currentPage}
            key={page} onClick={currentPage = () => {this.setState.currentPage = page}}//this.setState({ currentPage: page }) }
     
            >{page}</button>
        ))}
        {1 < pages.length && (
        <button type="button" className="btn btn-light"
        onClick={this.changeProx(currentPage)}
        >Próximo</button>
       )
       }
    </div>
